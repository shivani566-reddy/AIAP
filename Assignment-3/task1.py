...
if __name__ == "__main__":
    ...
    def calculate_energy_bill(units: float, rate_per_unit: float) -> float:
        """
        ...
        ----------
        units : float
            The number of energy units consumed.
        rate_per_unit : float
        ...
        """
        return units * rate_per_unit  # Calculate total bill

    # Example energy bill calculation
    units_consumed = 150  # Example units consumed
    rate = 0.15  # Example rate per unit
    total_bill = calculate_energy_bill(units_consumed, rate)  # Calculate total bill
    print(f"Total Energy Bill for {units_consumed} units at ${rate}/unit: ${total_bill:.2f}")